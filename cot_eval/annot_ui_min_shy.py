...
def draw_points(img_path: str, pts: Dict[str, Optional[Tuple[int, int]]]) -> Image.Image:
    ...
    for name, color in [("gt", (0, 255, 0, 200))]:  # Only draw ground truth points
        p = pts.get(name)
        if isinstance(p, (tuple, list)) and len(p) == 2 and all(isinstance(v, int) for v in p):
            y, x = p
            r = max(60, int(min(im.size) * 0.1))  # Increased radius by 10x
...
def main():
    ...
    ap.add_argument("--port", type=int, default=7863, help="Port number for the web interface (default: 7860)")
...
# python annot_ui_min_shy.py --input /data1/home/donglingzhong/codespace/AgentCPM-GUI-Evaluation/cot_eval/data/AgentCPM-GUI/android_control_high_test/sampled.jsonl

# python annot_ui_min_shy.py --input /data1/home/donglingzhong/codespace/AgentCPM-GUI-Evaluation/cot_eval/data/UI-TARS-1.5-7B/android_control_high_test/sampled.jsonl

# python annot_ui_min_shy.py --input /data1/home/donglingzhong/codespace/AgentCPM-GUI-Evaluation/cot_eval/data/GUI-Owl-7B/android_control_high_test/sampled.jsonl