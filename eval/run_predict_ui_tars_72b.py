...
def _primary_device_from_map(hf_device_map):
    # Robustly find the lowest CUDA index from map values that can be str/int/torch.device
    dev_idxs = []
    for v in hf_device_map.values():
        if isinstance(v, int):
            ...
            if v == "cpu" or v == "disk":
                continue
            m = re.match(r"cuda:(\d+)", v)
            if m:
                dev_idxs.append(int(m.group(1)))
        elif isinstance(v, torch.device):
            if v.type == "cuda":
                dev_idxs.append(v.index if v.index is not None else 0)
    if dev_idxs:
        return torch.device(f"cuda:{min(dev_idxs)}")
    return torch.device("cuda", 0) if torch.cuda.is_available() else torch.device("cpu")

def get_primary_device():
    global _llm
    if hasattr(_llm, "hf_device_map") and isinstance(_llm.hf_device_map, dict):
        return _primary_device_from_map(_llm.hf_device_map)
    if hasattr(_llm, "device"):
        return _llm.device
    return torch.device("cuda", 0) if torch.cuda.is_available() else torch.device("cpu")

def _init_llm(model_name, tp=1, max_gpu_mem="76GiB"):
    global _llm,_tokenizer
    if _llm is None:
        if tp and tp > 1:
            # Shard 72B across multiple GPUs using integer keys for accelerate
            visible = torch.cuda.device_count()
            use_tp = min(tp, visible) if visible > 0 else 0
            if use_tp <= 0:
                raise RuntimeError("No CUDA devices available for sharded inference.")
            max_memory = {i: max_gpu_mem for i in range(use_tp)}  # integers, not "cuda:i"
...
def move_to(device):
    ...
    # When sharded, skip .to()
    if hasattr(_llm, "hf_device_map"):
        return "Model sharded across GPUs (device_map=auto); skip .to()"
    _llm = _llm.to(device)
    if _tokenizer is None:
...
def uitars2minicpm(action_str):
    ...
    Args:
        action_str (str): full LLM output text containing Thought and Action
    Returns:
...
def predict(args, datasets):
    ...
    use_sharding = args.tensor_parallel_size and args.tensor_parallel_size > 1

    if use_sharding:
        # Initialize one sharded model across multiple GPUs in this process
        _init_llm(args.model_path, tp=args.tensor_parallel_size, max_gpu_mem=args.max_gpu_mem)
        ...
        for dataset in datasets:
                ...
                for episodes_file in episodes_files:
                    ...
                    for index,episode in enumerate(episodes):
                        ...
                        for prev_episode in episodes[:index]:
                            # ...existing code building episode_history...
            ...
            with open(output_file, "w", encoding="utf-8") as f_out:
                print("Predicting")
                for task_value in tqdm(all_tasks, total=len(all_tasks), dynamic_ncols=True):
                    try:
                        episode = run_episode(*task_value)
    ...
    # Fallback: original multi-process (replicated model per GPU)
    with ProcessPoolExecutor(max_workers=len(DEVICES),initializer=_init_llm,initargs=(args.model_path,)) as poolexec:
        ...
        for t in tasks:
            print(t.result())
        # ...existing code for loading episodes, scheduling poolexec.submit(run_episode,...), writing outputs...
        print(f"Prediction saved at: {output_file}.")
    os.system(f"cat {args.output_dir}/*/predict.jsonl > {args.output_dir}/all.jsonl")
    print(f"Merged prediction saved at: {args.output_dir}/all.jsonl.")

def postprocess_predictions(output_dir: str):
    """
    Fix existing predict.jsonl files in-place using the improved uitars2minicpm,
    then rebuild all.jsonl. Creates .bak backups before overwriting.
    """
    predict_files = []
    for root, _, files in os.walk(output_dir):
        for fn in files:
            if fn == "predict.jsonl":
                predict_files.append(os.path.join(root, fn))

    if not predict_files:
        print(f"No predict.jsonl files found under: {output_dir}")
        return
    ...
    for pf in predict_files:
        ...
        fixed = 0
        total = 0
        with open(bak, "r", encoding="utf-8") as fin, open(pf, "w", encoding="utf-8") as fout:
            for line in fin:
                line = line.strip()
                if not line:
                    continue
                try:
                    ...
                    # keep original line if it is not valid json
                    fout.write(line + "\n")
                    continue
                total += 1
                ...
                if isinstance(out_text, str):
                    try:
                        obj["pred"] = uitars2minicpm(out_text)
                        fixed += 1
                    except Exception as e:
                        print(f"Postprocess parse error in {pf}: {e}")
                fout.write(json.dumps(obj, ensure_ascii=False) + "\n")
        print(f"Fixed {fixed}/{total} entries in {pf}")

    # rebuild all.jsonl
    all_path = os.path.join(output_dir, "all.jsonl")
    if os.path.exists(all_path) and not os.path.exists(all_path + ".bak"):
    ...
    with open(all_path, "w", encoding="utf-8") as fout:
        for pf in predict_files:
            with open(pf, "r", encoding="utf-8") as fin:
                for line in fin:
                    fout.write(line)
    print(f"Merged prediction saved at: {all_path}.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--postprocess_only", action="store_true",
                        help="Only fix existing predict.jsonl files under output_dir without re-running model")
    # New flags for 72B sharding
    parser.add_argument("--tensor_parallel_size", type=int, default=int(os.getenv("TP_SIZE", "4")),
                        help="Shard the 72B model across this many GPUs (use 4 for four A100s)")
    parser.add_argument("--max_gpu_mem", type=str, default=os.getenv("MAX_GPU_MEM", "76GiB"),
                        help="Per-GPU memory budget for sharding, e.g., 76GiB for A100 80GB")
    ...
    if args.postprocess_only:
        postprocess_predictions(args.output_dir)
    else:
        predict(args, data_subset)

# CUDA_VISIBLE_DEVICES=0,1,2,3 python run_predict_ui_tars_72b.py --model_path /path/to/72B --tensor_parallel_size 4 --max_gpu_mem 76GiB