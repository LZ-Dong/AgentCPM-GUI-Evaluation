...
def read_episodes_from_jsonl(jsonl_file_path, train_episodes, output_test_dir, output_train_dir):
    ...
    with open(jsonl_file_path, 'r', encoding='utf-8') as file:
        for line in tqdm(file, total=total_lines, desc="saving data"):
            ...
            if episode_id in train_episodes:
                ...
                episode_dir = os.path.join(output_train_dir, str(episode_id))
                os.makedirs(episode_dir, exist_ok=True)
...
output_train_dir = os.path.join(os.path.join(current_dir, 'android_control_train/train/android_control'))

train_episodes = read_train_list(train_json_path)
episodes = read_episodes_from_jsonl(jsonl_file_path, train_episodes, output_test_dir, output_train_dir)

print(f"Number of train episodes: {len(episodes['in_train'])}")
print(f"Number of test episodes: {len(episodes['not_in_train'])}")
print(f"Save train set at: {output_train_dir}")
